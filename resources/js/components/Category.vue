  ...
  <div>
    <div class="card">
      <div class="card-header">
        <h5>Products Categories Management</h5>
      ...
      <div class="card-body table-responsive">
        ...
        <div class="alert alert-info" v-else>
          No Product Categories List yet
        </div>
      </div>
    </div>
    <div
    ...
    >
      <div class="modal-dialog animate-top" role="document">
        <div class="modal-content">
          ...
          <form
            @submit.prevent="updateMode ? updateCategory() : storeCategory()"
          >
...
export default {
  ...
  methods: {
    ...
    updateCategory() {
      this.$Progress.start();
      this.form
        .put("/admin/api/category/" + this.form.id)
        .then((resp) => {
    ...
    loadCategories() {
      axios.get("/admin/api/category").then((resp) => {
    ...
    deleteCategory(category) {
      this.$swal({
        ...
        if (result.value) {
          this.$Progress.start();
          axios
            .delete("/admin/api/category/" + category.id)
            .then((resp) => {
              this.$Progress.finish();
              const index = this.categories.findIndex(
                (s) => s.id === category.id
